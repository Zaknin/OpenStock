...
The deterministic reference is `Investment Portfolio Tracker - TMOAP v8.9(3).xlsx`, inspected read-only during Phase 0. Source SHA-256 at inspection start: `190E3031C68A5BBFDA3BF98541B8A3C4A6C2115433C7CB7FC3555A4456FA2A00`.
...
9. **Instrument identity and aliases.** The workbook contains both exchange-qualified and unqualified symbols. Define canonical ID, exchange/mic, provider aliases, delisting history, and collision handling without exposing private symbols in tracked artifacts.
...
12. **Current snapshot date.** Cached values in the reference workbook must be treated as frozen fixtures, not current market data. The exact dates belong only in the private fixture pack.
