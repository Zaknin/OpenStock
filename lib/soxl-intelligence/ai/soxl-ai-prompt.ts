...
export function buildSoxlAiPrompt(
    ...
    const userInstruction = [
        ...
        'Every evidenceRefs array must contain unique aliases only, must not repeat an alias within the same array, must contain no more than 8 aliases, and must contain only aliases that directly support that specific item.',
        'summary.evidenceRefs must contain at least 1 and no more than 8 aliases. Use only the strongest evidence needed to support the summary.',
        'Every other generated item evidenceRefs array must contain at least 1 and no more than 6 aliases.',
        ...
        'An output that exceeds the specified reference count is invalid.',
